...
Each step is red before it is green, except a step marked **guard**: a
case the earlier steps already satisfy, kept so it fails the day the
constraint goes; a guard names the mutation that makes it red.
...
## Risks and decisions
Open points, each with the decision made for this task and the reason, one
paragraph per decision, each alternative rejected in one line. A point
decided here is the planner's; the criteria do not call it a local choice
of the implementer.
