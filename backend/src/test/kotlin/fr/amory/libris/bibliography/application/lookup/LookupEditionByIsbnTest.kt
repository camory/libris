...
class LookupEditionByIsbnTest {
    ...
    fun `no source knowing the ISBN answers that it is unknown`() {
        // Given
        val lookup = LookupEditionByIsbn(listOf(LookupAnswering(NothingKnown), LookupAnswering(NothingKnown)))
    ...
    fun `every source having failed answers that no source replied`() {
        // Given
        val lookup = LookupEditionByIsbn(listOf(LookupAnswering(Failed), LookupAnswering(Failed)))
    ...
    fun `one source failing while the other knows nothing answers that the ISBN is unknown`() {
        // Given
        val lookup = LookupEditionByIsbn(listOf(LookupAnswering(Failed), LookupAnswering(NothingKnown)))
